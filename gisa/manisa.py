...
def float_to_word(number):
    ...
    result = "{0} {2} {1}".format(int_to_word(
        int(integer_part)), int_to_word(int(decimal_part)), WORD_SPLIT)
    return result
...
def int_to_word(number, grade=[False, False]):
    ...
    units = ["iray", "roa", "telo", "efatra",
             "dimy", "enina", "fito", "valo", "sivy"]
    tens = ["folo", "roapolo", "telopolo", "efapolo", "dimapolo",
            "enimpolo", "fitopolo", "valopolo", "sivifolo"]
    hundreds = ["zato", "roanjato", "telonjato", "efajato",
                "dimanjato", "eninjato", "fitonjato", "valonjato", "sivinjato"]
    thousands = ["arivo", "alina", "hetsy"]
    big = ["tapitrisa", "miliara", "lavitrisa"]

    result = ""
    # Convert number to list of digits and reverse order
    ...
    for i in range(len(digits)):
        ...
        if i == 0 and digit == 0:
            ...
            grade = [True, grade[1]]
            result = result + ("" if result == "" else " sy ") + \
                int_to_word((number // 10**6) % 10**9, grade) + " " + big[0]
        elif i == 9 and digit != 0 and not grade[1]:
            grade = [grade[0], True]
            result = result + ("" if result == "" else " sy ") + \
                int_to_word((number // 10**i) % 10**12, grade) + " " + big[1]
        elif i == 12 and digit != 0:
            result = result + ("" if result == "" else " sy ") + \
                int_to_word(number // 10**12, [False, False]) + " " + big[2]
    return result
